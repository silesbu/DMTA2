...
ndcg <- function(x) {
  ...
  dcg_obtained <- x[,"relevance"] / log2((1:nrow(x))+1)
  dcg_ideal <- x[order(x$relevance,decreasing=TRUE),"relevance"] / log2((1:nrow(x))+1)
  
  sum(dcg_obtained) / sum(dcg_ideal)
 }
...
setwd("C:/Users/11837543/Desktop/Data")
...
rm(nas, booked, clicked, factor_cols, i, len.nas, na.vec, name, position)
...
underTest <- read.csv("underTest.csv",header=TRUE)
...
rf_train$relevance <- as.factor(rf_train$relevance)

rm(all_train, na.df, no.na_train, underTrain)
```
...
<!-- ```{r} -->
<!-- library("StatRank") -->
<!-- Evaluation.NDCG(ranking$prediction, ranking$relevance) -->
