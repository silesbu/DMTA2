...
file_train <- file.path(Working_directory, "full_train" , "full_train.csv") 
#file_test <- file.path(Working_directory, "Data Mining VU data" , "test_set_VU_DM_2014.csv")

# Opening the files 
all_train <- read.csv(file_train, header = TRUE)
...
# Stablishing the working directory and the file paths 
Working_directory <- setwd("~/DM_Basic_Assignment/Assignment_2")
mod_train <- file.path(Working_directory, "modified_splits_dm", "mod_train.csv")
mod_test <- file.path(Working_directory, "modified_splits_dm", "mod_test.csv")
mod_validation <- file.path(Working_directory, "modified_splits_dm", "mod_valid.csv")
...
file_underTrain <- file.path(Working_directory, "Balanced_Data", "Balanced_Data", "underTrain.csv")
underTrain <- read.csv(file_underTrain, header = TRUE)

file_Test <- file.path(Working_directory, "modified_splits_dm", "mod_test.csv")
Test <- read.csv(file_Test, header = TRUE)
 
# file_underValidatioin <- file.path(Working_directory, "Balanced_Data", "Balanced_Data", "underValidation.csv")
...
```{r}
#Select columns with less than a 50% of NAs
variables_without_nas <- na.df$attribute[na.df$missing_values<=0.50]
variables_without_nas <- as.character(variables_without_nas)
c <-  c("orig_destination_distance", "visitor_location_country_id", "prop_location_score1", "srch_adults_count", "srch_children_count", "srch_room_count", "visitor_hist_starrating", "visitor_hist_adr_usd")
variables_without_nas <- variables_without_nas[!variables_without_nas %in% c]
no.na_train <- underTrain[variables_without_nas]

#Delete variables that are not in the test set
lr_train <- subset(no.na_train, select=c(-srch_id,-position,-click_bool,-booking_bool, -ignored_bool))

#Convert variables to type factor 
lr_train$relevance <- as.factor(lr_train$relevance)
...
gbm.fit <- gbm(relevance ~ ., data = lr_train, distribution = "multinomial")
summary(gbm.fit)

pred_logi <- predict(gbm.fit, newdata = Test, type = "response", n.trees = 100)
pred_logi <- as.data.frame(pred_logi)

predic <- 5*pred_logi$`5.100`+ pred_logi$`1.100`

df <- data.frame(srch_id= Test$srch_id, prop_id= Test$prop_id, prediction=predic, relevance=Test$relevance)
...
a <- c("starrating_diff", "usd_diff", "prop_starrating_monotonic")
b <- as.character(variables_without_nas)
todo <- c(a,b)
no.na_train <- underTrain[todo]
...
df <- data.frame(srch_id= Test$srch_id, prop_id = Test$prop_id, prediction=pred_forest_vali, relevance = Test$relevance)
...
set.seed(42)
system.time({ranger_fit = train(relevance ~ ., data = rf_train,
                                method = "ranger",
                                num.threads = 1,
                                tuneGrid = rf_grid)})
...
set.seed(42)
et_fit = train(relevance ~ ., data = rf_train,
               method = "extraTrees",
               trControl = ndcg_mean,
               tuneGrid = et_grid,
               numThreads = 4)
