...
#Train <- subset()
variables_with_nas <- na.df$attribute[na.df$missing_values<=0.75] # Select columns with less than a 75% of NAs

rf_train <- subset(train, select=c(-srch_id,-position,-click_bool,-gross_bookings_usd,-booking_bool,variables_with_nas))


#rf.train <- data.matrix(rf.train)
...
rf_model <- randomForest(relevance ~ . , data=rf_train, importance=TRUE,ntree=1000, na.action = na.exclude(rf_train))
varImpPlot(rf_model)
