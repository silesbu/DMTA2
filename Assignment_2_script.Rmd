...
nas <- is.na(underTrain)

len.nas <- length(nas[,1])

# Initialize NA vector
na.vec <- vector(mode="integer",length=length(colnames(underTrain)))
...
na.df <- data.frame(colnames(underTrain),na.vec)
...
variables_without_nas <- na.df$attribute[na.df$missing_values<=0.05]
no.na_train <- underTrain[as.character(variables_without_nas)]

#Delete variables that are not in the test set
rf_train <- subset(no.na_train, select=c(-srch_id,-position,-click_bool,-booking_bool))
```
...
rf_model <- randomForest(relevance ~ . , data=rf_train, importance=TRUE)
varImpPlot(rf_model)
...
# ##Prediction
# #* Validation set
# pred_forest_vali <- predict(forest_model_vali, newdata = validation, type = "class")
# 
# 
# 
# 
# fit <- randomForest(x = Train[,-c(1,2)], y = as.factor(Train[,2]))
# 
# rankRF <- function(prop_id) {
#     ## given a query Id, it will return a dataframe with the results of the
#     ## query ordered based on their probability of being relevant. The top rows
#     ## in the dataset correspond to those with higher probability.
# 
#     tmpdf <- test[test$prop_id == prop_id,]
# 
#     prediction <- predict(rf_model, newdata = tmpdf[,-c(1:4)], type = "prob")[,2]
# 
#     ## generating dataset with relevant columns
#     rankdf <- data.frame(prop_id = tmpdf$prop_id, r.bin = tmpdf$relevance,
#                          docid = tmpdf$docid, pred = as.numeric(prediction))
# 
#     ## sorting based on probability of being relevant
...
glm.fit <- glm(relevance ~ . , data = rf_train, family = binomial)
